...
const CompanyArena: React.FC = () => {
  ...
  const fetchRankings = async () => {
    try {
        const data = await apiFetch(`/api/owner/arena/stats?period=${activePeriod}&metric=${activeMetric}`);
  ...
  return (
    <div className="space-y-8 animate-fadeIn font-display">
      <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6 mb-10">
        <div>
          <h1 className="text-xl lg:text-3xl font-black text-white uppercase tracking-tighter">Company Arena</h1>
          <p className="text-zinc-500 text-[10px] lg:text-xs uppercase tracking-[0.2em] mt-1">Global Organization Leaderboards</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {['ALP', 'Dials', 'Retention', 'Lead Ratio', 'Profitability'].map((m) => (
          <button
            ...
            className={`p-4 border text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-all text-center ${
      ...
      <div className="bg-zinc-950 border border-zinc-900 rounded-none overflow-hidden shadow-2xl">
        <div className="p-6 lg:p-8 border-b border-zinc-900 bg-zinc-900/30 flex justify-between items-center">
          <h3 className="text-[10px] lg:text-xs font-black uppercase tracking-widest text-white">
            Top Performers <span className="text-[#D4AF37] ml-2">[{activePeriod}]</span>
          </h3>
          <div className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Tracking: {activeMetric}</div>
        </div>
          ...
          <table className="w-full text-left min-w-[600px]">
            <thead>
              <tr className="border-b border-zinc-900">
                <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-zinc-500 font-black">Rank</th>
                <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-zinc-500 font-black">Agent</th>
                <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-zinc-500 font-black text-right">{activeMetric}</th>
                <th className="px-8 py-5 text-[10px] uppercase tracking-widest text-zinc-500 font-black text-center">Trend</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-900">
              {rankings.map((agent) => (
                <tr key={agent.rank} className="hover:bg-zinc-900/30 transition-colors group">
                  ...
                  <td className="px-8 py-6 text-sm font-black text-white uppercase tracking-tight group-hover:text-[#D4AF37] transition-colors">{agent.name}</td>
                  <td className="px-8 py-6 text-xs text-white text-right font-black font-mono tracking-tighter">
                    {getMetricValue(agent)}
                  </td>
                  <td className="px-8 py-6 text-center">
                    {agent.trend === 'up' && <span className="text-green-500 text-sm">▲</span>}
                    {agent.trend === 'down' && <span className="text-red-500 text-sm">▼</span>}
                    {agent.trend === 'steady' && <span className="text-zinc-600 text-sm">●</span>}
                  </td>
                </tr>
              ))}
            </tbody>
